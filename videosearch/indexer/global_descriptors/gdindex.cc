...
void GDIndex::generate_index_shot_based(const vector<string>& feature_files, 
    ...
    // This vector is used in the custom shot mode to list the indices of the
    // frames in each shot
    vector < vector < uint > > frames_in_shot;
    ...
    } else if (shot_mode == SHOT_MODE_CUSTOM_AGG) {
        // This will generate only a global signature per shot, with a custom list of frames per shot
        number_shots = shot_beg_frames[0];
        number_entries_in_index = number_shots;
        uint shot_beg_frames_idx = 1;
        for(uint count_shot = 0; count_shot < number_shots; count_shot++) {
            uint num_frames_in_shot = shot_beg_frames[shot_beg_frames_idx];
            shot_beg_frames_idx++;
            vector < uint > frames_in_this_shot;
            for(uint idx = 0; idx < num_frames_in_shot; idx++) {
                frames_in_this_shot.push_back(shot_beg_frames[shot_beg_frames_idx] - 1);
                shot_beg_frames_idx++;
            }
            frames_in_shot.push_back(frames_in_this_shot);
        }
    } else {
...
#pragma omp parallel for       
    for (uint count_shot = 0; count_shot < number_shots; count_shot++) {
        uint number_frames_this_shot;
        if (shot_mode != SHOT_MODE_CUSTOM_AGG) {
            // Note: the number calculated in the following lines is a simple
            // subtraction, since the next number is not part of the shot, so 
            // there's no +1 at the end of the calculation
            if (count_shot != number_shots - 1) {
            ...
            number_frames_this_shot = frames_in_shot[count_shot].size();
        }
        ...
        if (shot_mode != SHOT_MODE_CUSTOM_AGG) {
            ...
            frames_to_use = frames_in_shot[count_shot];
        }
...
#pragma omp critical 
        ...
        } else if (shot_mode == SHOT_MODE_SHOT_AGG || shot_mode == SHOT_MODE_CUSTOM_AGG) {
